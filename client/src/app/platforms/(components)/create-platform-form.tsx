...
export function CreatePlatformForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    startTransition(async () => {
      const res = await createPlatformAction(values);
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card>
        <CardHeader>
          <CardTitle className="text-center">Create Platform</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
